...
export class TokenOrder implements IOrder {
  ...
  public async start(): Promise<void> {
    ...
    console.log(
      chalk.yellow(`[TokenOrder => ${this.token.name}] started ! Refreshing...`)
    );
  ...
  public async refreshBalances() {
    try {
      ...
      if (this.balance.toString() !== "0") {
        this.balanceInWBNB = await this.contracts.getTokenPriceInWBNB(
          this.address,
          formatUnits(this.balance, this.token.decimals)
        );
        this.balanceInBUSD = await this.contracts.getWBNBPriceInBUSD(
          formatEther(this.balanceInWBNB)
        );

        this.logInfo(
          `No balance for this token... [ ${formatUnits(
            this.balance,
            this.token.decimals
          )} ${this.token.symbol} ] = [${formatEther(
            this.balanceInWBNB
          )} WBNB] = [${formatEther(this.balanceInBUSD)} BUSD]`
        );
      } else {
        const balanceInWBNB = await this.contracts.getTokenPriceInWBNB(
          this.address,
          "1"
        );
        const balanceInBUSD = await this.contracts.getWBNBPriceInBUSD(
          formatEther(balanceInWBNB)
        );

        this.logInfo(
          `No balance for this token... [ 1 ${
            this.token.symbol
          } ] = [${formatEther(balanceInWBNB)} WBNB] = [${formatEther(
            balanceInBUSD
          )} BUSD]`
        );
      }
